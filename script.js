...
const cartEnabledPackageIds = new Set(["simple-elegant", "on-the-go", "flashmob", "emotions", "unique-wow"]);
...
const getCartItemsText = () =>
  cart
    .map((item) => `- ${getCartItemText(item)} - ${formatCurrency(item.price * item.quantity)}`)
    .join("\n");
...
const renderCakeSummary = () => {
  ...
  cakeSummary.innerHTML = `
    ...
    <div><dt>Final price</dt><dd>${price ? formatCurrency(price) : "Choose weight"}</dd></div>
  `;
};
...
const openDetails = (packageId) => {
  ...
  modalCartButton.textContent = canAddPackageToCart(packageId) ? "Add to Cart" : "Order Now";
...
if (cakeCheckoutButton) {
  cakeCheckoutButton.addEventListener("click", () => {
    if (addCakeToCart()) {
      closeCakeModal();
      closeCart();
      openCheckout();
